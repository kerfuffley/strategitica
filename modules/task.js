...
function isOneTimeDaily(task, userTagNames) {
    if (Object.keys(userTagNames).length > 0) {
        ...
        if (oneTimeDailyTagId !== '') {
            return hasTag(task, oneTimeDailyTagId);
        }
    }
    else {
        return false;
...
function hasTimeOfDayTag(task, timeOfDay, userTagNames) {
    if (Object.keys(userTagNames).length > 0) {
        var timeOfDayTagId = '';
        ...
        if (timeOfDayTagId !== '') {
            return hasTag(task, timeOfDayTagId);
        }
    }
    else {
        return false;
 ...
 */
function duration(task, userTagNames) {
    var duration = 0;
 ...
 */
function badgeHtml(task, userTags) {
    ...
    var taskDuration = duration(task, userTags);
 ...
 */
function modalHtml(task, userTags) {
    var headerHtml = '';
    var bodyHtmlStatic = '';
    ...
    if (frequency(task) != '') {
        bodyHtmlStatic += '<tr class="task-param-static-js"><th>Frequency</th><td>' + frequency(task) + '</td></tr>';
    }
    if (task.type === 'daily') {
    ...
    if (Object.keys(userTags).length > 0) {
        ...
        Object.keys(userTags).forEach(function (key) {
            ...
            bodyHtmlEditable += '<input class="form-check-input task-tag-js" type="checkbox" id="task-' + task.id + '-tag-' + key + '" value="' + key + '"' + (hasTag(task, key) ? ' checked' : '') + '>';
...
export { hasTag, isOneTimeDaily, hasTimeOfDayTag, duration, frequency, badgeHtml, modalHtml, tooltipHtml };